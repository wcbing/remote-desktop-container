# Chinese Desktop Container with RDP support
...
> ⚠️ QQ 等 Electron/Chromium 应用，由于 Docker 的 namespace 支持较差，需要将 QQ 的 `.desktop` 文件中启动命令后加上 `--no-sandbox`，或者在你新建容器时加上 `--privileged` 选项（不推荐）。Podman 没有问题，用户可直接使用。
...
使用 `test` 用户登陆，密码是 `test`，端口为 43389，挂载 hostname 和 `test` 用户主目录。
> ⚠️ 挂载敏感目录时请小心。
